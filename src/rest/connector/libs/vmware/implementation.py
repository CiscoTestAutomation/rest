__author__ = "Sukanya Kalluri <sukkallu@cisco.com>"
...
class Implementation(Implementation):
    '''Rest Implementation for Vmware Vcenter
    ...
    ------------
        ...
        >>> testbed = loader.load('/users/xxx/xxx/vcenter.yaml')
        >>> device = testbed.devices['vcenter']
    ...
    def connect(self, timeout=10, port=443, protocol='https'):
        ...
        login_url = '{url}/rest/com/vmware/cis/session'.format(url=self.url)
        log.info("Connecting to '{d}' with alias "
                 "'{a}'".format(d=self.device.name, a=self.alias))

        self.session = requests.Session()
        self.session.auth = HTTPBasicAuth(username, password)
        response = self.session.post(login_url, headers = STD_HEADER, verify=self.verify)
        log.info(response)
        ...
        self.token = response.json()['value']  # Retrieve the Token from the returned JSONhahhah
